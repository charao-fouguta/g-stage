@section('title', 'About｜G-STAGE JAPAN株式会社')
@section('description', 'メンズファッション関連事業を国内4拠点、4店舗、オンラインショップ展開。縫製専門の高松市の自社工場は高品質で信頼性の高い国産品を生産。OEM•ODM事業も展開中です。')

@include('layouts.header')
<div class="l-main u-paddingSideTB50">
	<main>
		<div class="l-column__wide">
			<article>
				<section class="l-section l-pageTop">
				<h1><span>About<span class="u-rubyBottom">私たちについて</span></span></h1>
					<div class="l-breadcrumb">
						<a href="{{ route('top') }}">Home</a>
						<div>About</div>
					</div>
				</section>
				<section>
					<div class="c-wideBanner">
						<figure><img sizes="(max-width:375px), (min-width:768px)" srcset="{{ asset('images/page-mv-about-s.jpg') }} 375w, {{ asset('images/page-mv-about-l.jpg') }} 768w" src="{{ asset('images/page-mv-about-s.jpg') }}" alt=""></figure>
					</div>
					<div class="l-columnRow l-column__nallow c-passage__pattern2 u-flex25-75__PC">
						<div class="u-flex22-78__PC">
							<h2>Company<br>Profile</h2>
							<p>会社概要</p>
						</div>
						<dl class="p-profile">
							<div>
								<dt>商号</dt>
								<dd>G-STAGE・JAPAN株式会社</dd>
							</div>
							<div>
								<dt>所在地</dt>
								<dd>
									<dl class="p-profile">
										<div>
											<dt>東京本社</dt>
											<dd>
												<div>〒150-0001　東京都渋谷区神宮前3-22-10 SAITO BLDG.4F-A</div>
												<div>TEL:03-6434-9580　FAX:03-6434-9581</div>
												<a href="#tokyo" class="cboxMap1 c-button__broad">MAP</a>
												<div class="lib-cboxMap">
													<div id="tokyo" class="lib-mapWrap">
														<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3241.27434766266!2d139.70489021525844!3d35.67024618019684!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x60188ca3687f3aeb%3A0x2e018dce98ce7bde!2z44CSMTUwLTAwMDEg5p2x5Lqs6YO95riL6LC35Yy656We5a6u5YmN77yT5LiB55uu77yS77yS4oiS77yR77yQIOaWieiXpOODk-ODqyA0Rg!5e0!3m2!1sja!2sjp!4v1617084542577!5m2!1sja!2sjp" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy"></iframe>	
													</div>
												</div>
											</dd>
										</div>
										<div>
											<dt>大阪支店</dt>
											<dd>
												<div>〒540-0026 大阪市中央区内本町2-3-11 内本町グリーンビル2F</div>
												<div>TEL:06-6947-1407　FAX:06-6947-7100</div>
												<a href="#osaka" class="cboxMap1 c-button__broad">MAP</a>
												<div class="lib-cboxMap">
													<div id="osaka" class="lib-mapWrap">
													<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3280.8697725069997!2d135.51053641523166!3d34.68323608043895!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x6000e7245aea8279%3A0xea489bc6ba516e2!2z5YaF5pys55S644Kw44Oq44O844Oz44OT44Or!5e0!3m2!1sja!2sjp!4v1617094278076!5m2!1sja!2sjp" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy"></iframe>
													</div>
												</div>
											</dd>
										</div>
										<div>
											<dt>滋賀物流</dt>
											<dd>
												<div>〒529-1207　滋賀県愛知郡愛荘町岩倉937-2</div>
												<div>TEL:0749-37-0077　FAX:0749-37-0078</div>
											</dd>
										</div>
										<div>
											<dt>高松工場</dt>
											<dd>
												<div>〒761-0445　香川県高松市西植田町636-7</div>	
												<div>TEL:087-849-1338　FAX:087-849-1338</div>
											</dd>
										</div>
									</dl>
								</dd>
							</div>
							<div>
								<dt>設立</dt>
								<dd>2000年2月</dd>
							</div>
							<div>
								<dt>資本金</dt>
								<dd>7,000万円</dd>
							</div>
							<div>
								<dt>取引先銀行</dt>
								<dd>三菱東京UFJ銀行 / りそな銀行 / 滋賀銀行</dd>
							</div>
							<div>
								<dt>主要事業</dt>
								<dd>製品卸販売 / OEM•ODM事業 / EC事業 / アパレル物流 / 縫製工場</dd>
							</div>
						</dl>
					</div>
				</section>
				<section>
					<div class="c-wideBanner u-verticalM100">
						<figure><img sizes="(max-width:375px), (min-width:768px)" srcset="{{ asset('images/page-mv-about2-s.jpg') }} 375w, {{ asset('images/page-mv-about2-l.jpg') }} 768w" src="{{ asset('images/page-mv-about2-s.jpg') }}" alt=""></figure>
					</div>
					<div class="l-columnRow l-column__nallow c-passage__pattern2 u-flexHalf__PC">
						<div>
							<h2>TAKAMATU Shirt<br>sewing factory</h2>
							<p class="u-verticalM__pattern2">高松シャツ工房</p>
							<p class="u-ff-18">高松市でシャツ縫製専門の自社工場を運営。<br>メイド・イン・ジャパンのオリジナルブランドのシャツ生産や、OEM•ODM事業を展開</p>
						</div>
						<div class="u-square">
							<div>
								<figure><img sizes="(max-width:375px), (min-width:768px)" srcset="{{ asset('images/takamatsu-Shirt-sewing-factory-s.jpg') }} 375w, {{ asset('images/takamatsu-Shirt-sewing-factory-l.jpg') }} 768w" src="{{ asset('images/takamatsu-Shirt-sewing-factory-s.jpg') }}" alt="高松シャツ工房"></figure>
							</div>
						</div>
					</div>
				</section>
			</article>
		</div>
	</main>
</div>
@include('layouts.footer')