	...
	<main>
		<div class="l-column__nallow u-paddingSide18">
			<section class="l-section l-pageTop" id="contact">
				...
				<form action="{{ route('contact.confirm') }}" method="post" id="validate_form">
					@csrf
					<div class="p-form__input">
						<p class="p-form__title">お名前<span class="u-caution">※</span></p>
						<input type="text" name="name" placeholder="例) 木永　いずみ" class="p-form__name" value="{{ old('name') }}">
						@error('name')
							<p class="u-colorRed">{{ $message }}</p>
						@enderror
						<p class="p-form__title">電話番号</p>
						<input type="text" name="phone_number" placeholder="例) 06-6947-7055" class="p-form__tel" value="{{ old('phone_number') }}">
						@error('phone_number')
							<p class="u-colorRed">{{ $message }}</p>
						@enderror
						<p class="p-form__title">メールアドレス<span class="u-caution">※</span></p>
						<input type="text" type="email" name="email" placeholder="例) info@g-stage1967.com" class="p-form__mail" value="{{ old('email') }}">
						@error('email')
							<p class="u-colorRed">{{ $message }}</p>
						@enderror
					...
					<div class="p-form__admission">
						<input type="checkbox" name="check" id="check"><label for="check">個人情報の取り扱いについてに同意する</label>
						@error('check')
							<p class="u-colorRed">{{ $message }}</p>
						@enderror
					</div>
